...
DBAppendCmd::exec(const string& option)
{
   ...
   size_t pos0 = myStrGetTok(option,key);
   size_t pos1 = myStrGetTok(option,value,pos0);
    ...
    else if(!myStr2Int(value,tempValue))
    {
       cerr << "Error: Illegal option!! (" << value << ")" << endl;
       return CMD_EXEC_ERROR;
    }
    else
    {
      
      DBJsonElem temp(key, tempValue);
